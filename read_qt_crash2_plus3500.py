...
#           qtend[:10] = 0.0 
        #perturb_data_path = f"{data_buffer_path}/stend.bin"
        #if os.path.isfile(perturb_data_path):
        #    os.remove(perturb_data_path)
        #    os.remove(f"{data_buffer_path}/qtend.bin")
        #    print("stend.bin, qtend.bin removed")
#
#        if step >= 10 and step < 20:
#            # let SPCAM run normally for 10 steps and then add perturbation for 10 steps
...
#            stend.tofile(f"{data_buffer_path}/stend.bin")
        if step % 8 == 0:
            target_file = f"{READ_QT_PATH}/*{idx_to_filename(step+3500)}"
            print(target_file)
            print(glob.glob(f"{READ_QT_PATH}/*{idx_to_filename(step+3500)}"))
            qt_file = glob.glob(f"{READ_QT_PATH}/*{idx_to_filename(step+3500)}")[0]
...
if __name__ == "__main__":
    ...
    #              )
      # run experiment
      # run_cesm()
      run_experiment(None, online_data_path, data_buffer_path, qtend_post_process)
      # cleanup()
