...
class AutoencoderResMLP(nn.Module):
    ...
    def forward(self, x): # (Q,T,ps,dqls, dtls, qtend,stend, radiation_related, cloud, lwup)t-1, (Q,T,ps,dqls,dtls)
        ...
        if self.resmlp_flag:
            # using only current input data
            # this by mistake, using reconstructed data as input
            x_resmlp = x_rec[:, -self.input_size:, :, :] # Q, T, ps, dqls, dtls of current step
