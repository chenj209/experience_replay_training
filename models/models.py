...
class ResMLP(nn.Module):
    def __init__(self, input_size, output_size, m, activation, num_blocks):
        ...
        self.outlayer = nn.Linear(m, output_size, bias=True)
        if activation=='relu':
            self.activation = nn.ReLU(inplace=True)
        elif activation== 'tanh':
            self.activation = nn.Tanh()
...
class ResNet_output30_RH(nn.Module):
    def __init__(self, m, activation, num_blocks):
        super(ResNet_output30_RH, self).__init__()
        self.fc1 = nn.Linear(152, m)
        self.input_size = 152
        self.fc2 = nn.Linear(m, m)
