...
def train(batch, model, criterion, optimizer, device):
    ...
    points_x, points_y = (points_x.float()).cuda(), (points_y.float()).cuda(device)
...
def train_penalty(batch, model, criterion, optimizer):
    ...
    # print(outputs_y.size(), points_y.size())
    loss = criterion(outputs_y, points_y)
    loss_update = loss + 10*torch.mean(torch.nn.functional.relu(-outputs_y))
    # print('omg')
    # compute gradient and do SGD step
    optimizer.zero_grad()
