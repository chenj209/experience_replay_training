...
#               batch_size, lr_strategy, lr, epoch, noise_std, weight_decay, dropout)
#           print(name)
#           commands = f"CUDA_VISIBLE_DEVICES=2 screen python online_training.py --data_dir {DATA_DIR}" + ' --output_type 30-59 --noise_std {} ' \
#                      '--network {} --node_size {} --num_blocks {} --activation {} --dropout {} ' \
#                      '--train-batch {} --lr_strategy {} --lr {} --epoch {} --wd {} ' \
#                      '--checkpoint online_ckpt/{} --resume {}'.format(str(noise_std),
#                                                     network, node_size, num_blocks, activation,
...
#
                    ...
                    #    batch_size, lr_strategy, lr, epoch, noise_std, weight_decay, dropout)
                    print(name)
                    commands = f"CUDA_VISIBLE_DEVICES=3 python online_training.py --data_dir {DATA_DIR}" + " --output_type 61-65 --noise_std {} " \
                               '--network {} --node_size {} --num_blocks {} --activation {} --dropout {} ' \
                               '--train-batch {} --lr_strategy {} --lr {} --epoch {} --wd {} ' \
                               '--checkpoint online_ckpt/{} --resume {}'.format(str(noise_std),
                                                              network, node_size, num_blocks, activation,
