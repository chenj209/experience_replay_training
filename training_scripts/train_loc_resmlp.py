...
def main(args):
    ...
    model_struc = learnable_vector.LearnableLocationResMLP
    model = model_struc(
        config=ae_config,
    ...
    """
    warmup_epochs = ae_config["warmup_epochs"]
    warmup_start_lr = ae_config["warmup_start_lr"]
...
#             else:
        ...
        #     print('!!!!!!!!!!!!!!!!!batch',points_x.size())  #1024 122???
            ...
            test_loss = criterion(outputs_y, points_y).item()
            test_losses[0].update(test_loss, batch[0].size(0))
