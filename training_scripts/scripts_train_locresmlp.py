...
if __name__ == "__main__":
    ...
    name = f"locresmlp_model029_sampled1_0313_multistep{args.multistep}_weight{args.pred_weight}_{args.rec_weight}_{args.ae_config.rstrip('.json')}_EX_{'+'.join(ex_input)}_EXPREV_{'+'.join(ex_input_prev)}"
            ...
            for node_size in ['512']:
                for noise_std in [0.0]:
                    for lr in [str(args.lr)]:
                        ...
                        print(name)
                        commands = f"python train_loc_resmlp.py --data_dir {DATA_DIR}" + " --output_type 0-29 --noise_std {} " \
                                '--network {} --node_size {} --num_blocks {} --activation {} --dropout {} ' \
                                '--train_batch {} --lr_strategy {} --lr {} --epoch {} --wd {} ' \
                                '--checkpoint /pscratch/sd/c/chenjd21/ckpts_time/{} --multistep {} --sample_rate 1 --workers 24 --region_mask {} ' \
