...
if __name__ == "__main__":
    ...
    name = f"{BASELINE_CKPT_PREFIX}6165_sampled{SAMPLE_RATE}_multistep{args.multistep}_EX_{'+'.join(ex_input)}_EXPREV_{'+'.join(ex_input_prev)}_region_{args.region_mask.split('/')[-1].rstrip('.npy')}"
            ...
            for node_size in ['512']:
                for noise_std in [0.0]:
                    for lr in [str(args.lr)]:
                        ...
                        print(name)
                        commands = f"python train_time_model_ex.py --data_dir {DATA_DIR}" + " --output_type 61-65 --noise_std {} " \
