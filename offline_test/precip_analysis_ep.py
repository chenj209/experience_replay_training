...
def plot_precip_hist(hist_pred, hist_gt):
    ...
    # Plot using semilogy (automatically sets y-axis to log scale)
    plt.semilogy(bins, hist_pred, label='Prediction', color='blue', linewidth=2)
    plt.semilogy(bins, hist_gt, label='Ground Truth', color='red', linewidth=2)
...
if __name__ == "__main__":
    ...
    model = load_resmlp_newformat2(model_ckpt_path, 309, 30)
    model.eval()
    gt_list = []
    raw_list = []
    pred_list = []
    ...
    plot_precip_hist(hist_pred*100, hist_gt*100)
    plt.savefig("precip_hist_ep.png")
    plt.show()
