...
def print_min_max_by_var(data, target_vars, col_names, delevelwise=False, reduce_lvl=0):
    """
    Print the mean and standard deviation of the data for each variable in target_vars.
    ...
    for col in target_vars:
        ...
        if data_range == 30:
            data_range = data_range - reduce_lvl
        print(col, data[:,cur_idx:cur_idx+data_range].min(), \
              data[:,cur_idx:cur_idx+data_range].max())
        cur_idx += data_range