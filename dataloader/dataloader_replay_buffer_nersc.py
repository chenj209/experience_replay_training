...
class DatasetDisk(data.Dataset):
    ...
    def __init__(
        ...
        if self.multistep > 0:
            ...
            self.file_names = self.all_files
    ...
    def __len__(self):
        ...
        # return self.size
        return (len(self.file_names) + self.sample_stride - 1) // self.sample_stride
    ...
    def load_data_ex(self, index):
        ...
        next_file = "/".join(tokens[:-1]+[idx_to_filename(target_fileidx+1, suffix=".npz")])
        tx_next = self.load_slice_ex(next_file, self.input_indices)
        next_y = self.load_slice_ex(next_file, self.output_indices)
        next_x = np.concatenate([tx, y, tx_next], axis=0)
        ...
        if self.is_train and self.noise_std>0:
            ...
            noise_x_next = np.random.randn(next_x.shape[0]) * self.noise_std
            noise_y_next = np.random.randn(next_y.shape[0]) * self.noise_std
            x = x + noise_x
            y = y + noise_y
            next_x = next_x + noise_x_next
            next_y = next_y + noise_y_next


        return x, y, next_x, next_y, file_names[::-1]
    ...
    def load_data(self, index):
        ...
        # load current data and previous data up to self.multistep
        target_file = self.file_names[index]
        file_names = [target_file]
        if not os.path.exists(target_file):
            raise ValueError(f"File {target_file} does not exist")
        ...
        next_file = "/".join(tokens[:-1]+[idx_to_filename(next_fileidx)])
        next_prev_inputs = []
        for p in range(self.multistep, 0, -1):
            prev_file = "/".join(tokens[:-1]+[idx_to_filename(next_fileidx-p)])
            tx_prev = self.load_slice(prev_file, self.prev_input_indices)
            next_prev_inputs.append(tx_prev)
            file_names.append(prev_file)
        next_tx = self.load_slice(next_file, self.input_indices)
        next_x = np.concatenate([*next_prev_inputs, next_tx], axis=0)
        next_y = self.load_slice(next_file, self.output_indices)
    ...
    def get_index(self, index):
        ...
        x, y, next_x, next_y, file_names = self.load_data(index)
