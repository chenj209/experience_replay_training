...
from dataloader_landmask import DatasetDiskLandMask as DatasetDisk
...
def main(args):
    ...
    for epoch in range(args.epoch):
        ...
        for iter, batch in enumerate(trainloader):
            ...
            land_sea_mask = (batch[0][0,:,122]==land_sea_flag)
            batch[0] = batch[0][:, land_sea_mask, :122]
            if args.output_type == '0-29':
                batch[1] = batch[1][:, land_sea_mask, :30]
            if args.output_type == '30-59':
                batch[1] = batch[1][:, land_sea_mask, 30:60]
            if args.output_type == '60':
                batch[1] = batch[1][:, land_sea_mask, 60:61]
            if args.output_type == '61-65':
                batch[1] = batch[1][:, land_sea_mask, 61:66]
